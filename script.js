...
function drawGraph() {
    ...
    for (var name in graph.data) {
        ...
        for (var depIndex in obj.depends) {
            var link = {
                source: graph.data[obj.depends[depIndex].object],
                target: obj
            };
    ...
    graph.edgepaths = graph.svg.selectAll(".edgepath")
        .data(graph.force.links())
        .enter()
        .append('path')
        .attr({
            'd': function (d) { return 'M 0 0 0 0' },
            'class': 'edgepath',
            'fill-opacity': 0,
            'stroke-opacity': 0,
            'fill': 'blue',
    ...
    graph.edgelabels = graph.svg.selectAll(".edgelabel")
        .data(graph.force.links())
        .enter()
        .append('text')
        .style("pointer-events", "none")
    ...
    graph.edgelabels.append('textPath')
        .attr('xlink:href', function (d, i) { return '#edgepath' + i })
        .style("pointer-events", "none")
        .text(function (d, i) { 
            var index =  d.target.depends.findIndex(x => x.object === d.source.name);
            return d.target.depends[index].type;
...
function tick(e) {
    ...
    graph.edgelabels.attr('transform', function (d, i) {
    ...
    graph.edgepaths.attr('d', function (d) {
